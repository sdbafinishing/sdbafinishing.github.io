      ...
      <div class="card" style="padding:24px;">
        ...
        <div id="g-race" class="gs">
          ...
          <table class="gt">
            ...
            <tr><td>2c′</td><td>${ic('image')} <strong>Photo PNG</strong></td><td>Fast results-image button. Reads the pre-generated PNG from the results folder if present (instant); else generates it on the fly from the Joyi files (spinner); else falls back to the drag-drop picker. The PNG ( = results panel + auto-cropped finish strip) is also generated automatically in the background when the <code>.lcd</code> lands, and saved to the results share folder. See §5.</td></tr>
        ...
        <div id="g-photo" class="gs">
          ...
          <p><strong>Quick PNG (auto-generated) — ${ic('image')} Photo PNG button</strong></p>
          ...
          <ul>
            <li><strong>Auto-generated in the background</strong> — when the Joyi <code>.lcd</code> for a race lands (via the auto-watcher), RDMS renders a PNG = <em>results panel + finish strip auto-cropped around the reach points</em> (full strip if no <code>.jyd</code>) and saves it to the results share folder (<code>12 Output_Results/</code> + <code>80 Shared/{ref}_Output_Results/</code>) as <code>PhotoFinish_{ref}_R{N}.png</code>. Skipped if the PNG already exists.</li>
            <li><strong>${ic('image')} Photo PNG button</strong> on the race page resolves in this order: (1) read the saved PNG and show it instantly; (2) if none, generate on the fly from the Joyi files with a spinner, save, then show; (3) if no files found, open the drag-drop picker. The lightweight viewer has Download + Close.</li>
          </ul>
        ...
        <div id="g-racepage-extras" class="gs">
          ...
          <ul>
            ...
            <li><strong>Photo PNG button</strong> — a fast results-image button next to Photo Finish; reads the auto-generated PNG, generates on demand, or falls back to the picker (see §5).</li>
        ...
        <div id="g-sync" class="gs">
          ...
          <ul>
            ...
            <li><strong>Live web-viewer polling (pull).</strong> Sync is push-only (local &rarr; Supabase). To keep a left-open <em>online</em> tab current, the web viewer now <strong>pulls</strong> the selected event's race snapshots from Supabase every <strong>20&nbsp;s</strong> into its local store; the dashboard's own 10&nbsp;s render tick then repaints when something changed. This makes the two-tab workflow work — local app in one tab for operating, an online dashboard tab for monitoring. End-to-end lag is roughly the local push cycle (≤30&nbsp;s) plus the viewer poll (≤20&nbsp;s), so up to ~a minute behind — fine for monitoring, not instant. Polling is online-only, pauses while the tab is hidden, and (like the rest of the viewer) needs working Supabase reads. The <strong>digital flags + race number stay truly real-time</strong> via Firebase regardless.</li>
          </ul>
        </div>
